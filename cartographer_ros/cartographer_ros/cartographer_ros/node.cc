...
*/
  ...
  //ComputeRepeatedTopicNames给相同名字的topic加上序号
  for (const std::string& topic : ComputeRepeatedTopicNames(//遍历所有的laser_scan
           topics.laser_scan_topic, options.num_laser_scans)) {
    expected_topics.insert(SensorId{SensorType::RANGE, topic});//把SensorId加入到expected_topics中
  }
  for (const std::string& topic :
       ComputeRepeatedTopicNames(topics.multi_echo_laser_scan_topic,//遍历所有的multi_echo_laser_scan
                                 options.num_multi_echo_laser_scans)) {
    expected_topics.insert(SensorId{SensorType::RANGE, topic});
  }
  for (const std::string& topic : ComputeRepeatedTopicNames(//遍历所有的point_cloud2
  ...
  if (node_options_.map_builder_options.use_trajectory_builder_3d() ||
      (node_options_.map_builder_options.use_trajectory_builder_2d() &&
       options.trajectory_builder_options.trajectory_builder_2d_options()
           .use_imu_data())) {
    expected_topics.insert(SensorId{SensorType::IMU, topics.imu_topic});//把IMU的SensorId加入到expected_topics中
  }
  // Odometry is optional.
  if (options.use_odometry) {
    expected_topics.insert(
        SensorId{SensorType::ODOMETRY, topics.odometry_topic});//把odometry的SensorId加入到expected_topics中
  }
  // NavSatFix is optional.
  if (options.use_nav_sat) {
    expected_topics.insert(
        SensorId{SensorType::FIXED_FRAME_POSE, topics.nav_sat_fix_topic});//把nav_sat_fix的SensorId加入到expected_topics中
  }
  // Landmark is optional.
  if (options.use_landmarks) {
    expected_topics.insert(SensorId{SensorType::LANDMARK, kLandmarkTopic});//把landmarks的SensorId加入到expected_topics中
  }
...
int Node::AddTrajectory(const TrajectoryOptions& options,
  ...
  for (const auto& sensor_id : expected_sensor_ids) {//将所有订阅的topic主题存入subscribed_topics_
